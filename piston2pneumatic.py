...
replacements = [
    [r'A\[.*', r''], # remove all As
    [r'E\d.*', r''], # remove all Es
    [r'[G][1]\s\W', r''], # remove empty G1 statements
    [r'[G].*[Z].*', r'\nM9\n\g<0>'], # insert close valve
    [r'[G].*[Z].*', r'\g<0>\nM8\n'], # insert open valve
    [r'G0',r'M9\n\g<0>'],
    [r'M8\s+M9', r'\n'],
    # [r'\w\[.', r''], # remove all As
...
# def compile_replacements2():
    # for regexString, replacement in replacements2:
        # regex = re.compile(regexString, flags=re.IGNORECASE)
        # regMatch.append([regex, replacement])
